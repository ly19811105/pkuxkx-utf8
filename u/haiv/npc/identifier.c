...
int ask_tiaozhan()
{
      ...
      if(!wizardp(me) && time() - me->query("baixiaosheng/tz_time") < 600)
	     ...
	     if (me->query("antirobot/deactivity") > 2) message("vision",HIR"百晓生瞪圆了眼睛对你说到：“又他奶奶的是你！我看你是机器人吧。”\n"NOR,me);
...
int do_answer(string arg)
{
    ...
    if (arg==(string)me->query_temp("baixiaosheng/randomcode") )
    	{
			write("百晓生微微颔首道：“口令无误，请接受挑战吧，10分钟之内完成慕容，萧峰，胡一刀，护镖任务各一次，现在开始，马上行动。”\n");
...
int ask_finish()
{
      ...
      if(time() - me->query("baixiaosheng/tz_time") > 600)
  	  ...
  	  log_file("static/bxs",sprintf("%s(%s) 在%s百晓生任务中获得一颗"+obj->name()+"！\n",  
