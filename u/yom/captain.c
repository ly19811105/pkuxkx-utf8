...
string ask_enter()
{
	...
	ob = this_object();
	room = environment(ob);
	if(!room || room->query("short") != HIY"永安门"NOR)
	{
		ob->kill_ob(me);
		return "大胆刁民，竟敢欺骗本官！\n";
	}
...
void ask_nightout()
{
    ...
    command("say 这天好冷啊，兄弟们夜里得弄点东西暖暖身子才好");
    command("look " + me->query("id"));
    me->set_temp("marks/bjnightout",1);
}
int accept_object(object who, object ob)
{
    if (ob->query("money_id") && ob->value() >= 2000 && ob->query("marks/bjnightout"))
    {
    command("smile " + ob->query("id"));
    command("say 下次还来找我就对了。");
        ob->move("/d/beijing/dadao_yongan_s");
    ob->set("marks/bjnightout",0);
    }
}
