...
//                me->improve_skill(skill,skill_gain/4);
           ...
           if(!me->query("renduermai")&&(me->query_skill(skill,1)>299)&&(random(mykar)+random(myint)+random(mycon))/3>(15+random(10)))  
	{
                 tell_object( me, HIG"你突然觉得体内的真气流动不同寻常，体内任督二脉\n"
			"正在慢慢地融合......\n\n"NOR );
                          if( ((int)me->query_skill(skill,1)-300)/100+random(1000)>998)
		{
