...
void init()
{
       ...
       if ((int)this_player()->query_skill("jinwu-dao", 1)>0)
        {
        	  this_player()->set_skill("lieyan-dao",this_player()->query_skill("jinwu-dao",1));
            this_player()->delete_skill("jinwu-dao");
            tell_object(this_player(),"明教金乌刀法已经改成烈焰刀法。\n");
            "/cmds/skill/enable.c"->main(me,"blade lieyan-dao");
            "/cmds/skill/enable.c"->main(me,"parry lieyan-dao");
            "/cmds/skill/wbei.c"->main(me,"shenghuo-shengong lieyan-dao");            
        }
