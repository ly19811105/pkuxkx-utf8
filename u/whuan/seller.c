...
int do_ding(string arg)
{
        ...
        switch( ctype )
        {//"dog", "hawk", "cat", "pig", "rat"
        	case "dog":
        		cname=cname+"狗";
        		cids = ({cid+" gou"}) + ({cid}) + ({"gou"});
        		break;
        	case "hawk":
        		cname=cname+"鹰";
        		cids = ({cid+" ying"}) + ({cid}) + ({"ying"});
        		break;
        	case "cat":
        		cname=cname+"猫";
        		cids = ({cid+" mao"}) + ({cid}) + ({"mao"});
        		break;
        	case "pig":
        		cname=cname+"猪";
        		cids = ({cid+" zhu"}) + ({cid}) + ({"zhu"});
        		break;
        	case "rat":
        		cname=cname+"鼠";
        		cids = ({cid+" shu"}) + ({cid}) + ({"shu"});
        		break;
        } 
        cname += NOR;
        pet = new(PET_OB);
