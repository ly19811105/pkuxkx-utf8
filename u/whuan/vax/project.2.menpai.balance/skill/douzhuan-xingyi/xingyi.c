...
int perform(object me, object target)
{
    ...
    object weapon;
	 ...
	 // dewolf 大弟子判定
    if( me->query("family/family_name")!="姑苏慕容" || !me->query("family/topone") )
        return notify_fail("只有本派大弟子才有权使用斗转星移对敌！\n");
    ...
    if(me->query("neili")*2 < random(target->query("neili"))){
        me->start_busy(3);
        target->start_busy(3);
        me->add("neili", -200);
        me->add("jing", -20);
