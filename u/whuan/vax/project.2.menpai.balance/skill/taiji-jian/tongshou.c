...
int perform(object me, object target)
{
	...
	msg+="$n大喜，愈战愈勇，施展轻功滴溜溜转到$N身后，伸手便来拿$N。\n";
       if( random((int)me->query("tongshou",1))>4)
	{
		if(WeaponType==1)
     ...
     if( random((int)me->query("combat_exp",1))*5>target->query("combat_exp",1))
		{
			if(WeaponType==1)
			{
		...
		else
		{
			...
			message_vision(msg,target);
          target->receive_wound("qi",target->query("eff_qi",1)/10, me);
        target->receive_damage("qi", target->query("eff_qi",1)/3, me);
