...
int perform(object me, object target)
{
    ...
    if( !target || !target->is_character() || !me->is_fighting(target) || !living(target) )
	 ...
	 // dewolf 大弟子判定
    if( me->query("family/family_name")!="少林派" || !me->query("family/topone") )
        return notify_fail("只有本派大弟子才有权使用立地成佛对敌！\n");
...
int perform2(object me, object target, int p)
{
    ...
    if (random(me->query_skill("force")) > target->query_skill("force") / 4 &&
            me->query_skill("wuying-jian", 1) > 299 ){                
        me->start_busy(1);
        target->start_busy(2);
        me->add("neili", -300);
        me->add("jing", -30);                
...
int perform3(object me, object target, int p)
{
    ...
    if (random(me->query_skill("force")) > target->query_skill("force") / 4 &&
            me->query_skill("wuying-jian", 1) > 449 )
    {
        me->start_busy(2);
