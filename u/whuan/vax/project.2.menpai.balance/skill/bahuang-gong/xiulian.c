...
int exert(object me, string arg)
{
        ...
        if( me->is_fighting() )  return notify_fail("战斗中进行修炼，好象只有神仙才能做到。\n");
	if ((string)me->query("family/family_name") != "灵鹫宫" )  return notify_fail("你现在不是灵鹫宫弟子，不能使用八荒六合唯我独尊功进行修炼！\n");
	if (!me->query("lingjiu/xiulian"))  return notify_fail("你没有返老还童，不需要进行修炼！\n");
...
void end_xiulian( object me )
{
	...
	else
	{
		...
		{
		for(int i=0; i<sizeof(skill_status); i++) 								me->set_skill(sname[i],svalue[i]+5);
