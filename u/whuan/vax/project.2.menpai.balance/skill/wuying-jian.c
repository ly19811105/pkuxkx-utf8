...
int valid_learn(object me)
{
        if ((int)me->query_skill("force", 1) < 300)
                return notify_fail("你的「基本内功」火候不够，无法学「无影剑法」。\n");
        if ((int)me->query_skill("yijin-jing", 1) < 300)
                return notify_fail("你的「混元一气功」火候不够，无法学「无影剑法」。\n");
...
int practice_skill(object me)
{
        ...
        if ((int)me->query("jingli") < 100)
                return notify_fail("你的精力不够练「无影剑法」。\n");
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
        ...
        if( random(damage_bonus/2) > victim->query_str() ) {
        	...
        	k = 2+random(3);
                ...
                victim->receive_wound("qi", k*(damage_bonus - 100) /((k-1)*2 ));
