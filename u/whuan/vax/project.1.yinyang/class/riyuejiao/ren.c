...
void die()
{
	...
	if( objectp(obj = me->query_temp("last_damage_from")) && ((obj->query_temp("riyue/杀任")))) 
	{
		 obj->set_temp("riyue/killer",1);
	}
	message_vision("任我行死了。\n",obj);
	corpse = new("/d/riyuejiao/obj/corpse");
	corpse->move("/d/riyuejiao/prison2");
	obj->move("/d/riyuejiao/prison2");
	destruct(me);
