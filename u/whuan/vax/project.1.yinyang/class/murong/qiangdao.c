...
void die()
{
    ...
    if(killer->query("murong/yuxi"))
    {
		...
		{	//经验越高越复杂
			...
			{
				msg+= "宝物被我藏在了" + TASK_D->place_belong(where) + "的" + where->query("short") + ",也不知道被没被人取走，你赶快去找吧！\n";
				inv = all_inventory(where);
				yuxi->move(where);
				if (sizeof(inv) > 0)
					if (!yuxi->move(inv[random(sizeof(inv))]))	//有obj则移到obj上，不成功直接放地上
						yuxi->move(where);
				message_vision(HIY+this_object()->name()+"说道：" + msg + NOR,this_object());
			}
			
		}
		else   yuxi->move(this_object());
    }
    ::die();
}
void unconcious() { die();}
