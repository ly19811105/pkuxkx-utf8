...
int do_huwei()
{
	...
	if (random(10)>3 && me->query_temp("ljjob") < i )
	{
		...
		me->kill_ob(obj);
		obj->kill_ob(me);
		me->add_temp("lingjiuhuwei/killed_number",1);
		me->start_busy(random((int)me->query_temp("lingjiuhuwei/killed_number"))*2);
	}
	else
		me->start_busy(5);
	//检查任务次数，是否满足要求
	if (me->query_temp("ljjob") >i && 
	!me->is_fighting())
	{
