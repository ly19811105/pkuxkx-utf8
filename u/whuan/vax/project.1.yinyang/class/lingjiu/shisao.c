...
{
               command("say " + RANK_D->query_respect(ob) + "已是灵鹫宫首领，不必再扫雪了！\n");
                return "你还是做护卫任务吧。\n";
}
         if  ((int)ob->query("family/generation") == 2 || ob->query("score") > 1500)
{
               command("say " + RANK_D->query_respect(ob) + "已是灵鹫宫总管，不必再扫雪了！\n");
                return "你还是做护卫任务吧。\n";
}
...
//防止任务道具，互相交换使用
          ...
          obn->move(ob);
	  return    HIW"我刚得到消息，所谓的名门正派，来了几个人挑战我派。\n"+
           "          你赶快去"HIG+where+HIW"护卫一下吧(huwei)。\n"+
           "          如果真的有人来挑战，消灭他们。"NOR; 
}
