...
{
        mapping lrn;
        string exert, str, *file, type_str, file_name;
		int is_user_skill;
        object sk;
        int i, j = 0;
	...
	{
		if (file_size(PLAYER_SKILL_D(arg)+".c") < 1)
		{
			return notify_fail("错误！你要查对技能文件已经消失，请通知巫师。\n");
		}
		else
		{
			type_str = PLAYER_SKILL_D(arg)->type();
			sk = load_object(PLAYER_SKILL_D(arg));
			file_name = PLAYER_SKILL_D(arg);
			is_user_skill = 1;
		}
	}
	else
	{
		type_str = SKILL_D(arg)->type();
		sk = load_object(SKILL_D(arg));
		file_name = SKILL_D(arg);
		is_user_skill = 0;
	}
	...
	if( stringp(exert = file_name->exert_function_file("")) ){
		file = get_dir(exert);
		if( sizeof(file) > 0){
	...
	if (is_user_skill)
	{
		file = get_dir(PLAYER_SKILL_D(arg)+"/");
		if( sizeof(file) > 0){
		write ((string)sizeof(file)+"/kungfu/player_skill/"+arg+"/\n");
			//file  = filter_array(file, (: sort_skill :) );
				j = sizeof(file);
		}
	}
	...
	{
		if( stringp(exert = file_name->perform_action_file(""))){
			file = get_dir(exert);
			if( sizeof(file) > 0){
				file  = filter_array(file, (: sort_skill :) );
				j = sizeof(file);
