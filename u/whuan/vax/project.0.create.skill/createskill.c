...
spe_skill["dodge"]	=exp_factor-50+spe_skillnum*5;
...
{
	spe_skill["force"]+=30+random(40);
	spe_skill["damage"]+=20+random(20);
	spe_skill["dodge"]-=20+random(20);
...
string result;
	...
	mkdir("/kungfu/player_skill/"+spe_skill_name);
	rm("/kungfu/player_skill/"+spe_skill_name+"/"+performs["name"]+"."+(string)spe_skillnum+".c");
	write_file("/kungfu/player_skill/"+spe_skill_name+"/"+performs["name"]+"."+(string)spe_skillnum+".c", result);

	//children(PLAYER_SKILL_D(spe_skill["skill_en_name"]))->clean_up();
	
	write("你长出了一口气，「"+spe_skill["skill_ch_name"]+"」的绝招「"+HIY+performs["ch_name"]+NOR+"」领悟完成了。\n");
}
