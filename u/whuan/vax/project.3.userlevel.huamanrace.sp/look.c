...
string gettof(object ob)
{
    object weapon;
    string skill_type;
    int attack_points, hide_level, parry_point, dodge_point;
    mapping prepare;
	...
	parry_point = COMBAT_D->skill_power(ob, "parry", SKILL_USAGE_DEFENSE);
	dodge_point = COMBAT_D->skill_power(ob, "dodge", SKILL_USAGE_DEFENSE);
    if( intp(hide_level = ob->query("hide_level")) && hide_level > 0)
        return tough_level((attack_points+parry_point+dodge_point)/3, hide_level);
    return tough_level((attack_points+parry_point+dodge_point)/3, 0);
}
