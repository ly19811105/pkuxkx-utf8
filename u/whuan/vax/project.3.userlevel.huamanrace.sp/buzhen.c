...
int exert(object me, object target)
{
	 ...
	 // dewolf 大弟子判定
    if( me->query("id") != DADIZI_D->query_name("taohua") )
        return notify_fail("只有本派大弟子才有权使用迷魂阵！\n");
...
*/
        ...
        if( me->query_skill("bihai-shengong", 1) < 100 )
        ...
        qmdj = SPECIAL_D->query_level(me, "wuxing-divination");
		if( !qmdj )
                return notify_fail("你的五行数术修为不够，还未领悟「奇门阵法」！\n");
        ...
        env->set("cost", qmdj * 50);
        env->set("long", desc);
        env->set("th_buzhen", 1);
		env->set("th_zhen_owner",me->query("id"));
        env->set("th_pozhen", qmdj* 200);
        ...
        call_out("remove_effect", 5 + qmdj*2 , env);
