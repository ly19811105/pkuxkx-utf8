...
string ask_special_skill()
{
	...
	if (!user->query_temp("mark/ok_"+s_skill))
	{
		...
		if (user->query("nation")!= "中原人士")
			return user->query("nation")+"怕是难以修习"+to_chinese(s_skill)+"("+s_skill+")的。";
		
		if (cur_lvl >= user->query_skill("sword",1)/20)
			return "你的基本功太差了，没有办法继续学习"+to_chinese(s_skill)+"。你先去加强修炼"+ to_chinese("sword") + "(sword)吧。";
			
		if (!SPECIAL_D->query_level(user, "master-dex"))
			return "你先要修习"+to_chinese("master-dex")+"才能修炼"+to_chinese(s_skill)+"。";
	...
	else if (user->query("level_used") < user->query("level"))
	{
		...
		if ((cur_lvl = SPECIAL_D->query_level(user, s_skill)) < 20 )
		{
			...
			message_vision(query("name")+"在$N耳边轻声说了几句御剑的法门。\n", user);
			tell_object(user, "你的"+to_chinese(s_skill)+"("+s_skill+")进步了！\n");
			tell_object(user, "你觉得御剑的能力也随之增强了。\n");
			return "真想再和小师妹一起练剑……";
		}
	}
	return "什么？";
