...
string query_yinyang(object me)
{
	...
	// 天地无用(resist_yinyang) 抵抗本身阴阳属性	
	if (enable_force_level <= MIN_SKILL_LEVEL + (int)SPECIAL_D->query_effect(me, "resist_yinyang"))
	{
...
float yinyang_factor(object me)
{
	...
	if (enable_force_level > MIN_SKILL_LEVEL)
	{
		...
		if (skill_yinyang != "没有")
		{
			...
			if (skill_yinyang == "少阴")
			{
				factor += SPECIAL_D->query_level(me, "master-shaoyin")/60.0;
			}
			else if (skill_yinyang == "太阴")
			{
				factor += SPECIAL_D->query_level(me, "master-taiyin")/60.0;
			}
			else if (skill_yinyang == "纯阴")
			{
				factor += SPECIAL_D->query_level(me, "master-chunyin")/60.0;
			}
			else if (skill_yinyang == "少阳")
			{
				factor += SPECIAL_D->query_level(me, "master-shaoyang")/60.0;
			}
			else if (skill_yinyang == "太阳")
			{
				factor += SPECIAL_D->query_level(me, "master-taiyang")/60.0;
			}
			else if (skill_yinyang == "纯阳")
			{
				factor += SPECIAL_D->query_level(me, "master-chunyang")/60.0;
			}
			// year factor
