...
string ask_special_skill()
{
	...
	if (!user->query_temp("mark/ok_"+s_skill))
	{
		...
		if (cur_lvl >= user->query_skill("bihai-shengong",1)/20)
			return "你的内功根基太差了，没有办法继续学习本门秘技"+to_chinese(s_skill)+"。你先去加强修炼"+ to_chinese("bihai-shengong") + "(bihai-shengong)吧。";
	...
	else if (user->query("level_used") < user->query("level"))
	{
		...
		if ((cur_lvl = SPECIAL_D->query_level(user, s_skill)) < 20 )
		{
			...
			message_vision(query("name")+"在$N耳边轻声说了几句五行相生相克的诀窍。\n", user);
			tell_object(user, "你的"+to_chinese(s_skill)+"("+s_skill+")进步了！\n");
			tell_object(user, "你的五行术有所提高。\n");
			return "桃花岛绝对不是外人可以随便擅创的！";
		}
	}
	return "什么？";
}
