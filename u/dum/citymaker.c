...
mapping room_short = ([]);
string room_head,city,current_path;
...
void init()
{
        if( wizardp(environment()) ) {
        	seteuid(getuid(environment()));
                add_action("do_makecity", "makecity");
...
int makeroom(int _row, int _col)
{
  ...
  if (_row>1)              	    {if (maps[_row-1][_col] == "|"   && maps[_row-2][_col]!=" ")     {exit["north"]=room_head+sprintf("_%d_%d",_row-2,_col);}}
  if (_row>1 && _col<_Mcol-1)       {if (maps[_row-1][_col+1] == "/" && maps[_row-2][_col+2]!=" ")   {exit["northeast"]=room_head+sprintf("_%d_%d",_row-2,_col+2);}}
  if (_col<_Mcol-1)           	    {if (maps[_row][_col+1] == "-"   && maps[_row][_col+2]!=" ")     {exit["east"]=room_head+sprintf("_%d_%d",_row,_col+2);}}
  if (_row<_Mrow-1 && _col<_Mcol-1) {if (maps[_row+1][_col+1] == "\\"&& maps[_row+2][_col+2]!=" ")   {exit["southeast"]=room_head+sprintf("_%d_%d",_row+2,_col+2);}}
  if (_row<_Mrow-1)                 {if (maps[_row+1][_col] == "|"   && maps[_row+2][_col]!=" ")     {exit["south"]=room_head+sprintf("_%d_%d",_row+2,_col);}}
  if (_row<_Mrow-1 && _col>1)       {if (maps[_row+1][_col-1] == "/" && maps[_row+2][_col-2]!=" ")   {exit["southwest"]=room_head+sprintf("_%d_%d",_row+2,_col-2);}}
  if (_col>1)                       {if (maps[_row][_col-1] == "-"   && maps[_row][_col-2]!=" ")      {exit["west"]=room_head+sprintf("_%d_%d",_row,_col-2);}}
  if (_row>1 && _col>1)             {if (maps[_row-1][_col-1] == "\\"&& maps[_row-2][_col-2]!=" ")   {exit["northwest"]=room_head+sprintf("_%d_%d",_row-2,_col-2);}}
  file_name=current_path+room_head+sprintf("_%d_%d.c",_row,_col);
  file=sprintf("/*\n * File    : %s (%s)\n",room_short[maps[_row][_col]],file_name);
        file+=sprintf(" * Author  : %s(%s@%s)\n * Date    : %s\n",this_player()->query("name"),this_player()->query("id"),MUD_NAME,ctime(time()));
        file+=@FILE_HEAD
...
1.编辑地图档, 范例如下：
-----------------------------------------------------------------
a.客房 b.厢房 c.走廊 d.大厅 e.厨房 f.花园 g.大门 h.内室
  g   b
  |  / \
b-c-b   b
  |  \ /
...
地图说明：   本地图档所加边框(---)，为解释地图所用，实际地图中没有。
             地图第一行为图例，采用<房间标识.房间名称>的方式，即a代
             表客房，地图中所有a均被处理成客房。 房间标识与房间名称
             之间用＂.＂分割，两个图例之间用＂,＂分隔。
...
3. 指令格式：makecity <地图档>  ( 最好把城市名作为地图档名称， 因为
	     Citymaker 在设置outdoors房间的时候使用地图档名作为房间
	     的所属城市。)
2. Citymaker 需要设定存放档案的路径： <目录名> (默认为当前所在目录)。
4. Citymaker 需要设定户外的房间：<户外房间标识,户外房间标识,...>
             所设定的房间将被设定为户外(默认为室内)。        
5. 如果以上设定无误，确认后 Citymaker 会批量生成城市档案。(确认后本程
	     序将直接改写重名文件，所以请注意你所者设置的存储目录。)
6. 本程序欢迎修改，但请保留原作者名。	     
