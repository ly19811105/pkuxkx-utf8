...
void do_change(object ob)
{
	...
	for (i=0;i<sizeof(skills);i++)
		if ( (SKILL_D(keys(skills)[i])->type()!="knowledge") && (temp<values(skills)[i]) )
			temp = values(skills)[i];
			
	if (temp==0)
		temp=10;
	skills = me->query_skills();
        factor = (50 + random(60));
        temp = factor * temp / 100; 
        for (i=0;i<sizeof(skills);i++)
...
/* copy hp */
        ...
        me->set("combat_exp",to_int(hp_status["combat_exp"]*(random(5)+6)/10));
		if (random(100) == 0)
