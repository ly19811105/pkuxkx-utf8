...
int vote(object me, object victim)
{
  ...
  if ((reason = (int)victim->query("vote/reason"))>0 && (reason!=V_ROBOT))
  {
    ...
    if ( victim->query("learn_xiake_gf") )
        return notify_fail(victim->name()+"正在侠客岛读侠客行，怎么会是机器人？\n");
  if( victim->query("robot_on") ) 
  {
     return notify_fail(victim->name()+"正在被青狼主法官审讯中。\n");
  }
 ...
 vv = (int) ("/cmds/std/vote")->valid_voters(me)/5;
  ...
  if (df>0)
  {
      shout( HIG "【表决】"+me->name()+"投票对" +victim->name()
            +"进行机器人审讯，还差"+sprintf("%d", df)+"票。\n" NOR);
  ...
  } else
  {
      ...
      tell_object(victim,"一只手把你抓了起来, 你眼前一阵黑....\n");
       victim->move("/d/wizard/judge_room");
      victim->apply_condition("vote_clear", -10);
  }
