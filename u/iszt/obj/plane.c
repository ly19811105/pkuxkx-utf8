...
int do_rob(string arg)
{
        ...
        this_object()->set("long", HIR"这是一架已经被"+this_player()->name()+"劫持的波音767飞机，看来想瞄(aim)着哪飞。\n"NOR);
        return 1;
}
...
int do_aim(string arg)
{
        ...
        me= this_player();
        if (!arg)
                return notify_fail("你要瞄准哪个目标？\n");
        if(target = find_player(arg))
                return notify_fail("你想撞"+target->name()+"，可惜目标太小，你在天上找不到……\n");
        if(arg != "wtc")
                return notify_fail("又没有什么深仇大恨，何必呢……去撞世界贸易中心(wtc)吧！\n");
        ob=this_object();
