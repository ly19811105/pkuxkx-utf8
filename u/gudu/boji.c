...
zhe->add("time_perf",me->query_condition("jingji")*120);
//return 1;
}
...
//command("wear all");
me->apply_condition("jingji",INTERVAL);
}
/*
                inv = all_inventory(me);
               for (i=0;i<sizeof(inv);i++)
...
*/
       ...
       me->add("jingji/kill",1);
                zhe->set_temp("apply/damage",  me->query_temp("apply/damage")/3*2);
zhe->kill_ob(me);
me->kill_ob(zhe);
remove_call_out("checking");
call_out("checking",2,zhe,me);
return 1;
}
int checking(object zhe,object me)
{
if(zhe->is_fighting())
{
if(me->query_temp("weapon")&&!zhe->query_temp("weapon")) zhe->set("unfair",1);
if(me->query_temp("secondary_weapon")&&!zhe->query_temp("secondary_weapon")) zhe->set("unfair",1);
remove_call_out("checking");
call_out("checking",2,zhe,me);
return 1;
}
...
add_action("do_change","dig");
add_action("do_change","reply");
add_action("do_change","p");
add_action("do_change","get");
add_action("do_change","beg");
add_action("do_change","get");
add_action("do_change","drop");
add_action("do_change","abandon");
add_action("do_change","fangqi");
...
if(me->is_busy())
{
write(HIR"还在busy，就想退？"NOR);
return 1;
}
if(random(10)<7)
{
me->start_busy(1);
write(HIR"你眼见自己将性命不保，顿生退意，心中一阵慌乱，岂料竞技者早已将你退路封死!\n"NOR);
write(HIR"你双眼一闭，暗到：罢、罢、罢，没想到我"+me->query("name")+"纵横江湖整整"+chinese_number(me->query("age"))+"载，今日竟要丧命于此!\n"NOR);
return 1;
}
...
if (me->is_busy())
{
zhe->add("time_perf",80);
write(HIR"你正忙着呢！\n"NOR);
if(!zhe->is_busy()&&random(zhe->query("randomvalue"))==1)
{
if(zhe->query("randomvalue")>2) zhe->add("randomvalue",-2);
...
if(zhe->query("randomvalue")>2) zhe->add("randomvalue",-1);
return 1;
}
...
{
if(random(4)==1) this_object()->add("time_perf",1);
this_object()->add("time_plus",1+random(2));
this_object()->set_temp("last_damage_from",this_player());
//if(random(10)==1)
if(random(this_object()->query("combat_exp")/2)>random(this_player()->query("combat_exp"))&&!this_object()->is_busy())
{
if(random(30)==1)
{
...
{
if(arg=="recover"||arg=="regenerate"||arg=="powerup")
return 0;
else
{
if (me->is_busy())
...
if(sscanf(arg,"%s",perf)!=1)
 return 1;
command("yun "+perf+" "+this_player()->query("id"));
me->start_busy(15);
}
if(zhe->query("randomvalue")>2&&random(2)==1) zhe->add("randomvalue",-1);
return 1;
}
if(arg=="yihun")
{
...
message_vision(HIR"竞技者受$N内力的激荡，向$N反震！\n"NOR,me);
me->start_busy(random(15));
return 1;
}
if(random(5)==1) zhe->add("time_perf",1);
return 0;
}
}
}
...
{
this_object()->add("time_perf",80);
write(HIR"你正忙着呢！\n"NOR);
return 1;
}
if(random(20)==1) this_object()->start_busy(0);
command("remove "+arg);
...
time_limit=time_limit+zhe->query("time_plus")*2-100;
if(time_limit<20) time_limit=20;
dt=time_limit/5;
...
if(j>200) j=200;
exp=exp/100*(100+j);
if(me->query_temp("weapon")&&!zhe->query("unwield")&&zhe->query_temp("weapon")) exp=exp/2*3;
//if(me->query_temp("weapon")&&!zhe->query_temp("weapon")) exp=exp/5*2;
exp=exp/5*2;
if(exp<me->query_temp("last_exp")/10*7) exp=me->query_temp("last_exp")/10*7;
if(level>7&&exp<me->query_temp("last_exp")/20*21) exp=me->query_temp("last_exp")/20*21/INTERVAL*(INTERVAL-me->query_condition("jingji"));
if(query("unfair")) exp=exp/(level+2)*2;
if(exp<1) exp=1;
potential=exp/40;
...
if(me->query_temp("xiangbo_level")>=last_level&&SECURITY_D->get_status(me)== "(player)" )
{
rm(JINGJIZHIWANG);
...
me->add("jingji/win",1);
if(exp>me->query("jingji/highestexp")) 
{
message_vision(MAG"恭喜！$N打破了自己单场奖励的最高记录!\n这是$N自己对自己极限的一次成功挑战！\n"NOR,me);
message_vision(HIY"$N将会得到本场奖励的十分之一作为额外的奖励！\n"NOR,me);
me->set("jingji/highestexp",exp);
me->set("jingji/highestpotential",potential);
exp=exp/10;
potential=potential/10;
me->add_temp("jingji_exp",exp);
me->add_temp("jingji_potential",potential);
message_vision(HIR"$N被额外奖励了"+chinese_number(exp)+"点经验和"+chinese_number(potential)+"点潜能！\n"NOR,me);
}
if(me->query_temp("xiangbo_level")>me->query("jingji/highestlevel"))
{
message_vision(MAG"恭喜！$N打破了自己连胜场数的最高记录!\n这是$N自己对自己极限的一次成功挑战！\n"NOR,me);
message_vision(HIY"$N将会得到到目前为止本轮奖励的十分之一作为额外的奖励！\n"NOR,me);
me->set("jingji/highestlevel",me->query_temp("xiangbo_level"));
exp=me->query_temp("jingji_exp")/10;
potential=me->query_temp("jingji_potential")/10;
me->add_temp("jingji_exp",exp);
me->add_temp("jingji_potential",potential);
message_vision(HIR"$N被额外奖励了"+chinese_number(exp)+"点经验和"+chinese_number(potential)+"点潜能！\n"NOR,me);
}
