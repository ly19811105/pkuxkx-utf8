...
protected void get_passwd(string pass, object ob)
{
	...
	if( crypt(pass, my_pass) != my_pass ) {
			if( wiz_level(ob->query("id")) >0)
                                  log_file("crack_wiz_log", sprintf(ob->query("id") + "被从"+ query_ip_name(this_player()) + "登陆的人在"+ctime(time())+"用密码尝试过。\n"));
...
void init_new_player(object user)
{
    ...
    user->set("channels", ({ "chat", "rumor", "tt", "hslj", "baowei" }) );
...
//功夫等级检查
        ...
        if (user->query("seen_help") == 0)
                write(HIR+"你还没有看过help newbie，那里面有大部分新手常问的问题如吃喝等的解答以及很多很好的建议，希望你能尽快仔细阅读它。\n"+NOR);
