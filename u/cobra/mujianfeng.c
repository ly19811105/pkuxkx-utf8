...
void create()
{
	set_name("木剑封", ({"mu jianfeng", "jianfeng"}));
...
protected void confirm_create(string arg, object me)
{
	...
	if(!write_file("/menpai/exist",file)) return 0;
	me->delete("family");
	me->delete_temp("marks/自创门派");
	me->delete_temp("marks/门派名称");
...
int check_legal_title(string arg , object me)
{
	...
	write("\n");
	message( "channel:" + "chat",HIW + "【新闻】：『" + me->query("title",1)+HIW+" 』"+me->query("name") + "于今日创立了"+me->query("自创门派/门派名称",1)+HIW+"，真是可喜可贺!"NOR,users());
//	obj=new("/d	
	return 1;
}
