...
void create()
{
	...
	set_skill("dodge", 200);
	set_skill("xiake-shenfa", 200);
	set_skill("xiake-shengong",200);
	set_skill("xiake-jianfa",200);
	set_skill("sword",200);
...
{
	if(!wizardp(a)&&a->query("combat_exp")>1000000&&
...
int FaLing()
{
	...
	tell_object(fit_user[i], YEL "如果你愿意和对方进行比试请你也对" + me
		->name() + "("+(string)me->query("id")+")"+ "下一次 biwu(accept) 指令。\n" NOR);
