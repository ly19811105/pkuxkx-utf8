...
//                 return notify_fail("你内息无法运至赤阳二穴，不能使出「绝户」招式。\n");
        ...
        {
                me->start_busy(random(pfm_time1));
                l_temp=80-(int)me->query_skill("bixue-xinfa", 1)/10;
        ...
        if (damage > 300 )
         {
          ...
          if (target->query("gender")== "男性")
          {
           ...
           {if (target->query("gender")== "女性")
               message_vision(HIW"$N被你一击命中，，不禁既羞又怒，$n居然对一女流之辈也用此阴毒招数！\n"NOR,target,me);
            else
