...
string ask_badman()
{
	...
	if (me->query("score") >= 1000 && me->query("class") == "officer")
        {	command("shake "+me->query("id"));
...
void do_answer(string arg)
{
	...
	else
	{
		...
		time = me->query("mud_age");
    		...
    		me->set("pin", 9);
    		me->set("title", YEL"朝廷九品官员 "HIM"普通女兵"NOR);
	...
	else
	{
		...
		time = player->query("mud_age");
    		...
    		if (player->query("gender") == "男性")
    			player->set("title",YEL"朝廷九品官员 "GRN"普通官兵"NOR);
    		else
    		    	player->set("title",YEL"朝廷九品官员 "HIM"普通女兵"NOR);
    		player->set("pin", 9);
    		message_vision("$N拍着$n的肩膀说道：恭喜你成为朝廷的一员。\n", this_object(), player);
    		player->save();
    return 1;
...
void recruit_apprentice(object ob)
{
	...
	title = all_color[pin - 1] + "朝廷" + chinese_number(pin) + "品官员 "NOR + title;
	ob->set("title", title);
...
string do_task()
{
        ...
        if (interactive(ob)
         && (int)ob->query_condition("beijing_xuncheng"))
         {
            ...
            return (RED"你这么快回来了，是不是有些地方只是马马虎虎看看？\n"NOR);
            }
...
//
	feizei = new("/d/beijing/npc/feizei");
	if (!feizei) return "BUG: 请通知巫师。\n";
	if (exp < 100000)
	{
	...
	position = feizei->query_quest();
	if (feizei->move(position["place"]))
	{
		me->set("chaoting/catchfeizei", 1);
		me->set_temp("feizei/gkill1", 1);
                return "飞贼" + feizei->short() + "在『" + position["short"] + "』出现，请速去将他捉拿归案！";
	}
